...
struct group_shared_reg_load_store_tilevec {
    ...
    template<int H, int W, int NW, gl_t GL> __device__ static void device_func(const GL &input, const GL &output) {
        ...
        G::load(shared_tile, input, {});
        __syncthreads();
        kittens::rv<float, 16*W, kittens::ducks::rv_layout::naive> reg_vec;
        G::load(reg_vec, shared_tile, {13, 0});
        G::add(reg_vec, reg_vec, 13.0f);
        __syncthreads();
        G::store(shared_tile, reg_vec, {7, 0});
        __syncthreads();
        G::store(output, shared_tile, {});
    }
};
...
void group::memory::tile::shared_to_register::tests(test_data &results) {
    ...
    // Tilevec tests
    sweep_size_2d<group_shared_reg_load_store_tilevec<float>, SIZE, SIZE, 1>::run(results);
    sweep_size_2d<group_shared_reg_load_store_tilevec<kittens::bf16>, SIZE, SIZE, 1>::run(results);
    sweep_size_2d<group_shared_reg_load_store_tilevec<kittens::half>, SIZE, SIZE, 1>::run(results);

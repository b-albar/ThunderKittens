...
struct test_load { // load with TMA, write out normally
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            if(threadIdx.x == 0) kittens::tma::expect_bytes(smem_semaphore, kittens::size_bytes<kittens::st<T, 16*H, 16*W>> * 2 * 2);
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                if(threadIdx.x == 0) kittens::tma::load_async(shared_tile[i][j], input, {a, b, i, j}, smem_semaphore);
            }
...
struct test_load_oob { // load oob memory via TMA
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            if(threadIdx.x == 0) kittens::tma::expect_bytes(smem_semaphore, kittens::size_bytes<kittens::st<T, 16*H, 16*W>> * 2 * 2);
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                if(threadIdx.x == 0) kittens::tma::load_async(shared_tile[i][j], input, {input.batch() + a, input.depth() + b, i + 2, j + 2}, smem_semaphore);
            }
            kittens::wait(smem_semaphore, (a*input.depth()+b)%2);
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                kittens::warp::store(output, shared_tile[i][j], {a, b, i, j});
...
struct test_store { // load normally, store with TMA
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                if(threadIdx.x == 0) kittens::tma::store_async_read_wait<3>(); // make sure next tile is ready for write
                kittens::warp::load(shared_tile[i][j], input, {a, b, i, j});
            }
            __syncwarp(); // mem must be visible before store
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                if(threadIdx.x == 0) kittens::tma::store_async(output, shared_tile[i][j], {a, b, i, j});
            }
        }
    }
};
...
struct test_store_add_reduce {
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                if(threadIdx.x == 0) kittens::tma::store_async_read_wait<6>(); // make sure next tile is ready for write
                kittens::warp::load(shared_tile[i][j], input, {a, b, i, j});
            }
            __syncwarp(); // mem must be visible before store
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                if(threadIdx.x == 0) kittens::tma::store_add_async(output, shared_tile[i][j], {a, b, i, j});
                if(threadIdx.x == 0) kittens::tma::store_add_async(output, shared_tile[i][j], {a, b, i, j});
            }
        }
    }
...
struct test_store_min_reduce {
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            ...
            __syncwarp(); // mem must be visible before store
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                if(threadIdx.x == 0) kittens::tma::store_min_async(output, shared_tile[i][j], {a, b, i, j}); // output is zero-initialized
                if(threadIdx.x == 0) kittens::tma::store_min_async(output, shared_tile[i][j], {a, b, i, j});
            }
        }
    }
};
...
struct test_store_max_reduce {
    ...
    __device__ static void device_func(const GL &input, const GL &output) {
        ...
        for(int a = 0; a < input.batch(); a++) for(int b = 0; b < input.depth(); b++) {
            ...
            __syncwarp(); // mem must be visible before store
            for(int i = 0; i < 2; i++) for(int j = 0; j < 2; j++) {
                if(threadIdx.x == 0) kittens::tma::store_max_async(output, shared_tile[i][j], {a, b, i, j}); // output is zero-initialized
                if(threadIdx.x == 0) kittens::tma::store_max_async(output, shared_tile[i][j], {a, b, i, j});
            }
        }
    }
};

...
namespace kittens::prototype::vm {
    ...
    template <
        ...
        typename Globals = mlp_micro_globals>
    struct DownOp {
        static constexpr int opcode = 2;
        static constexpr int EXPECTED_ARRIVAL_COUNT = 128;

        struct parsed_instruction {
            int start_block_idx, end_block_idx, reduction_block_idx, start_reduction_col, iters;
            __device__ inline parsed_instruction(typename Config::instruction_t &instruction) {
                start_block_idx = instruction[1];     // in units of 1 (0, 16, 32, ..., 2032)
                end_block_idx = instruction[2];       // in units of 1 (0, 16, 32, ..., 2032)
                reduction_block_idx = instruction[3]; // in units of hidden_dim=2048 (0, 2048, 4096, 6144)

...
#pragma unroll
  ...
  __device__ static constexpr int L_partial_sem_idx(int q_head_local_idx,
                                                    bool is_finished) {
    return (Q_HEADS_PER_INSTRUCTION * NUM_STAGES * 2) + q_head_local_idx * 2 +
           (is_finished ? 1 : 0);
  }
  ...
  __device__ static inline semaphore &
  O_partial_arrived(state<config> &s, int q_head_local_idx, int stage) {
    return s.semaphores()[O_partial_sem_idx(q_head_local_idx, stage, false)];
  }
  __device__ static inline semaphore &
  O_partial_finished(state<config> &s, int q_head_local_idx, int stage) {
    return s.semaphores()[O_partial_sem_idx(q_head_local_idx, stage, true)];
  }
  __device__ static inline semaphore &
  L_partial_all_arrived(state<config> &s, int q_head_local_idx) {
    return s.semaphores()[L_partial_sem_idx(q_head_local_idx, false)];
  }
  __device__ static inline semaphore &
  L_partial_all_finished(state<config> &s, int q_head_local_idx) {
    return s.semaphores()[L_partial_sem_idx(q_head_local_idx, true)];
  }
  __device__ static inline semaphore &final_O_ready(state<config> &s,
  ...
  get_L_partial_smem(state<config> &s, int q_head_local_idx) {
  ...
  get_O_partial_smem(state<config> &s, int q_head_local_idx, int stage) {
    int pid = s.pid(SHARED_DATA_PAGE);
    char *page_base_ptr = reinterpret_cast<char *>(s.pages[pid].data);
    char *head_base_ptr = page_base_ptr + q_head_local_idx * size_per_head;
  ...
  struct consumer {
    static __device__ void run(const Globals &g, state<Config> &s) {
      ...
      if (warpid() < Q_HEADS_PER_INSTRUCTION) {
        ...
        for (int i = 0; i < inst.num_partials; ++i) {
          int stage = i % NUM_STAGES;
          warp::wait(O_partial_arrived(s, q_head_local_idx, stage),
                     (i / NUM_STAGES) % 2);
  ...
  // Storer Warp: Responsible for storing data from shared memory back to global
  // memory.
  struct storer {
    static __device__ void run(const Globals &g, state<Config> &s) {
      parsed_instruction inst{s};
      if (warp::laneid() < Q_HEADS_PER_INSTRUCTION) {
        ...
        // atomicAdd(&g.Bar[{inst.layer_idx, opcode - 1, inst.q_head_start_idx +
        // q_head_local_idx}], 1);
      }

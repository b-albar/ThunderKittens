...
struct globals_t {
  ...
  using instruction_layout =
      ::kittens::prototype::vm::instruction_layout<config>;
  using timing_layout = ::kittens::prototype::vm::timing_layout<config>;

  using weights_t =
      gl<bf16, 1, -1, -1, hidden_dim,
         st_bf<matvec_block_size, 512>>; // assumed to be N by 2048 (X@W.T).
  using weights_big_indim_t =
      gl<bf16, 1, -1, -1, intermediate_dim,
         st_bf<matvec_block_size, 512>>; // assumed to be N by 2048 (X@W.T).

  using activations_t = gl<bf16, 1, 1, 1, hidden_dim, sv_bf<hidden_dim>,
                           sv_bf<head_dim>, sv_bf<matvec_block_size>>;
  using activations_big_indim_t =
      gl<bf16, 1, 1, 1, intermediate_dim, sv_bf<intermediate_dim>,
         sv_bf<hidden_dim>, sv_bf<matvec_block_size>>;
  using logits_t = gl<bf16, 1, 1, 1, -1, sv_bf<matvec_block_size>>;

  using norm_weights_t = gl<bf16, 1, 1, -1, hidden_dim, sv_bf<hidden_dim>,
                            sv_bf<matvec_block_size>>;
  using rope_table_t = gl<float, 1, 1, -1, head_dim, sv_fl<head_dim>>;
  using kv_cache_t = gl<bf16, -1, -1, -1, head_dim, sv_bf<matvec_block_size>,
                        tma::descriptor<st_bf<kv_block_size, head_dim>, 1>>;

  // max attention partials == sm_count
  using attn_out_intermediates_t =
      gl<float, 1, num_attention_heads, -1, head_dim, sv_fl<head_dim>>;
  using attn_lse_intermediates_t = gl<float, 1, 1, num_attention_heads, -1,
                                      sv_fl<((sm_count + 15) / 16) * 16>>;

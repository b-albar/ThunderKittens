...
struct matvec_pipeline {
  ...
  __device__ static inline semaphore &weights_arrived(state<Config> &s,
                                                      int stage) {
    return s.semaphores()[1 + stage];
  }
  __device__ static inline semaphore &weights_finished(state<Config> &s,
                                                       int stage) {
  ...
  __device__ static inline semaphore &outputs_finished(state<Config> &s,
                                                       int stage) {
    return s.semaphores()[1 + 2 * INPUT_PIPELINE_STAGES +
                          OUTPUT_PIPELINE_STAGES + stage];
  }

  __device__ static inline sv_bf<Globals::hidden_dim> &
  get_activations(state<Config> &s) {
    return *reinterpret_cast<sv_bf<Globals::hidden_dim> *>(
  ...
  // __device__ static inline void launcher_load_activations(state<Config> &s,
  // const Globals &g)
  // {
  //     if (laneid() == 0)
  //     {
  ...
  __device__ static inline void loader_loop(state<Config> &s,
                                            const Globals &g) {
    parsed_instruction inst{s};
    ...
    if (laneid() == 0) {
      ...
      for (int iter = 0; iter < inst.iters; iter++) {
        wait(weights_finished(s, input_stage),
...
#pragma unroll
        for (int i = 0; i < 4; i++) {
          ...
          auto &weight_chunk =
              reinterpret_cast<st_bf<16, 512> &>(s.pages[weight_page]);
  ...
  consumer_loop(state<Config> &s, const Globals &g, rv_t &activations_vec) {
    // Setup
    parsed_instruction inst{s};
    ...
    for (int i = 0; i < inst.iters; i++) {
      ...
      wait(weights_arrived(s, input_stage),
           (i % (2 * INPUT_PIPELINE_STAGES)) >= INPUT_PIPELINE_STAGES);
      wait(outputs_finished(s, output_stage),
           (i % (2 * OUTPUT_PIPELINE_STAGES)) < OUTPUT_PIPELINE_STAGES);
      st_bf<16, REDUCTION_DIM_PER_WARP> &weights =
          reinterpret_cast<st_bf<16, REDUCTION_DIM_PER_WARP> *>(
              s.pages[weight_page].ptr())[warpid() % WARPS_PER_PAGE];
      sv_fl<16> &out_smem = *reinterpret_cast<sv_fl<16> *>(
          (float *)s.scratch() + (32 * output_stage));
...
#pragma unroll
  ...
  __device__ static inline void storer_loop(state<Config> &s,
                                            const Globals &g) {
    parsed_instruction inst{s};
...
struct rms_matvec_pipeline
    : public matvec_pipeline<Config, Globals, parsed_instruction,
                             pipeline_specifics> {
  using pipeline =
      matvec_pipeline<Config, Globals, parsed_instruction, pipeline_specifics>;
  ...
  __device__ static inline sv_bf<Globals::hidden_dim> &
  get_rms_scale(state<Config> &s) {
    return *reinterpret_cast<sv_bf<Globals::hidden_dim> *>(
        s.pages[get_activation_page(s)].ptr(
            sizeof(sv_bf<Globals::hidden_dim>)));
  }
  ...
  __device__ static inline void loader_loop(state<Config> &s, const Globals &g,
    ...
    if (laneid() == 0) {
      ...
      tma::load_async<cache_policy::EVICT_LAST>(rms_scale, g.*RmsPtr,
                                                {layer_idx, 0}, sem);
    }
  ...
  // __device__ static inline void
  // launcher_load_rms_and_activations(state<Config> &s, const Globals &g, int
  // layer_idx)
  // {
  //     if (laneid() == 0)
  //     {
  ...
  __device__ static inline void consumer_loop(state<Config> &s,
                                              const Globals &g) {

    using sv_t = sv_bf<REDUCTION_DIM_PER_WARP>;
    auto &rms_scale_smem =
        reinterpret_cast<sv_t *>(&get_rms_scale(s))[warpid()];
    auto &activations_smem =
        reinterpret_cast<sv_t *>(&get_activations(s))[warpid()];
    ...
    if (laneid() == 0 && warpid() == 0) {
      ...
      // tma::load_async<cache_policy::EVICT_LAST>(activations, g.*ActPtr, {},
      // sem);
    }
    ...
    auto activations_vec =
        rms_norm<Config>(rms_scale_smem, activations_smem, g.rms_norm_eps,
                         (void *)((float *)s.scratch() +
                                  (32 * pipeline::OUTPUT_PIPELINE_STAGES)));

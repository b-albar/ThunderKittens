...
template <typename config, typename globals> struct attention_partial {
  ...
  using attn_fl_rt =
      rt_fl<16, LLAMA_1B_KV_BLOCK_SIZE>; // only 4 values are used
  using attn_bf_rt =
      rt_bf<16, LLAMA_1B_KV_BLOCK_SIZE>; // only 4 values are used
  using max_vec_rv =
      col_vec<rt_fl<16, LLAMA_1B_HEAD_DIM>>; // only 4 values are used
    ...
    if (row4idx % 2 == 0 && laneid < 16) { // rows 0~3 or 8~11
      if (row4idx / 2 == 0) {              // rows 0~3
        for (int j = 0; j < src.width; j++) {
          U2 tmp[2];
          tmp[0] =
              base_types::convertor<U2, T2>::convert(src.tiles[0][j].data[0]);
          ...
          move<U2>::sts(dst_ptr[local_row_idx] + sizeof(U) * (col_idx + 8),
                        tmp[1]);
        }
      } else { // rows 8~11
        for (int j = 0; j < src.width; j++) {
          U2 tmp[2];
          tmp[0] =
              base_types::convertor<U2, T2>::convert(src.tiles[0][j].data[1]);
          tmp[1] =
              base_types::convertor<U2, T2>::convert(src.tiles[0][j].data[3]);
          int col_idx = local_col_idx * 2 + j * 16;
          move<U2>::sts(dst_ptr[local_row_idx] + sizeof(U) * col_idx, tmp[0]);
          move<U2>::sts(dst_ptr[local_row_idx] + sizeof(U) * (col_idx + 8),
                        tmp[1]);
...
#pragma unroll
        for (int k = 0; k < dst.packed_per_tile; k++) {
          const int col_idx_x = (j * dst.tile_size_col) + ((k / 2) * 8) +
                                ((warp::laneid() % 4) * 2);
          const int col_idx_y = (j * dst.tile_size_col) + ((k / 2) * 8) +
                                ((warp::laneid() % 4) * 2) + 1;
  ...
  struct launcher {
    static __device__ void wait_for_kv(const globals &g, state<config> &s,
                                       parsed_instruction &inst) {
      s.record(TEVENT_AT_GMEM_WAIT);

      // Wait for the previous ops to finish (16 dims each, so 4 ops on the same
      // head)
    ...
    static __device__ void run(const globals &g, state<config> &s) {
      if (warp::laneid() == 0) {
        ...
        int total_attn_blocks =
            (seq_len + LLAMA_1B_KV_BLOCK_SIZE - 1) / LLAMA_1B_KV_BLOCK_SIZE;
        int blocks_per_partial =
            (total_attn_blocks + inst.num_partials - 1) / inst.num_partials;
  ...
  struct storer {
    ...
    store_o_skip(const globals &g, state<config> &s, int q_head_start_idx) {
    ...
    static inline __device__ void store_o_no_skip(const globals &g,
                                                  state<config> &s,
    ...
    static __device__ void run(const globals &g, state<config> &s) {
      ...
      if (laneid < GQA_RATIO && !skip_attn_reduction) {
        ...
        float tmp;
        uint32_t src_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(
      ...
      if (laneid < GQA_RATIO) {
        ...
        if (skip_attn_reduction) {
          atomicAdd(&g.Bar[{inst.layer_idx, OPCODE_AttentionReduction - 1, 0}],
                    1);
        } else {
          // Adding only at 0, 4, 8, ... should be sufficient for the reduction
          // op!
          atomicAdd(
              &g.Bar[{inst.layer_idx, opcode - 1, q_head_start_idx + laneid}],
              1);
        }

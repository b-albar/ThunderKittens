...
struct MatVecAddOp {
  ...
  struct parsed_instruction {
    ...
    __device__ inline parsed_instruction(
      ...
      reduction_block_idx =
          instruction[4]; // in units of hidden_dim=2048 (0, 2048, 4096, 6144)
  ...
  struct pipeline_specifics {
    ...
    static __device__ inline void load_iter(state<Config> &s, const globals &g,
                                            parsed_instruction &inst, int iter,
                                            int col_idx,
                                            st_bf<16, 512> &weight_chunk,
                                            semaphore &sem) {
    ...
    static __device__ inline void store(state<Config> &s, const globals &g,
                                        parsed_instruction &inst,
                                        int output_idx, int output_stage) {

      int block_idx = inst.start_block_idx + output_idx;

      sv_fl<16> &output_smem = *reinterpret_cast<sv_fl<16> *>(
          (float *)s.scratch() + (32 * output_stage));
      sv_bf<16> &output_smem_bf = *reinterpret_cast<sv_bf<16> *>(
          (float *)s.scratch() + (32 * output_stage));
      ...
      if (warp::laneid() == 0) {
        ...
        tma::store_add_async<cache_policy::EVICT_LAST>(
  ...
  struct loader {
    static __device__ void run(const Globals &g, state<Config> &s) {
      // Need to clear the first few elements of the scratch buffer, since we
      // are using atomicAdd later.
  ...
  struct consumer {
    static __device__ void run(const Globals &g, state<Config> &s) {
      ...
      sv_t &activations_smem =
          reinterpret_cast<sv_t *>(&pipeline::get_activations(s))[warpid()];

      warp::load(activations_smem, g.*InputActivationsPtr,
                 coord<>{inst.start_reduction_col +
                         warpid() * pipeline::REDUCTION_DIM_PER_WARP});
      warp::sync();
  ...
  struct storer {
    ...
    static __device__ void run(const globals &g, state<Config> &s) {
      ...
      if (laneid() == 0) {
        ...
        tma::store_async_wait(); // not just read wait! full wait! must be
                                 // visible in global!

        // asm volatile("fence.acq_rel.gpu;\n"); // possible we need sc here but
        // I don't think so.

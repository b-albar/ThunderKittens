...
/**
 * @brief Atomic add from a register tile to a destination array in global memory with a row-major layout.
 *
 * @tparam RT The register tile type with a row-major layout.
 * @tparam U The data type of the destination array.
 * @param[out] dst The destination array in global memory to add data into.
 * @param[in] src The source register tile to add data from.
 * @param row_stride[in] The stride in elements between rows in the destination array.
 */
template<int axis, ducks::rt::row_layout RT, ducks::gl::all GL, ducks::coord::tile COORD=coord<RT>>
__device__ inline static void atomic_add(const GL &dst, const RT &src, const COORD &idx) {
    using T2 = RT::dtype;
    using U = typename GL::dtype;
    ...
    for(int i = 0; i < src.height; i++) {
        ...
        for(int j = 0; j < src.width; j++) {
            ...
            atomic<U2>::addg((U2*)(&dst_ptr[(row_0to3+0)*row_stride + col]), transfers[0]);
            atomic<U2>::addg((U2*)(&dst_ptr[(row_0to3+4)*row_stride + col]), transfers[1]);
        }
        #pragma unroll
        for(int j = 0; j < src.width; j++) {
            int col = j*src.tile_size_col + warphalf*8 + 2*(laneid % 4);
            ...
            transfers[1-warphalf] = packed_shfl_sync(MASK_ALL, transfers[1-warphalf], laneid^16);
            atomic<U2>::addg((U2*)(&dst_ptr[(row_0to3+ 8)*row_stride + col]), transfers[0]);
            atomic<U2>::addg((U2*)(&dst_ptr[(row_0to3+12)*row_stride + col]), transfers[1]);
        }
    }
}

/**
 * @brief Atomic add from a register tile to a destination array in global memory with a column-major layout.
 *
 * @tparam RT The register tile type with a column-major layout.
 * @tparam U The data type of the destination array.
 * @param[out] dst The destination array in global memory to add data into.
...
__device__ inline static void atomic_add(const GL &dst, const RT &src, const COORD &idx) {
    ...
    for(int i = 0; i < src.height; i++) {
        ...
        for(int j = 0; j < src.width; j++) {
            ...
            atomic<U>::addg(&dst_ptr[(row+0)*row_stride + (col+0)], base_types::convertor<U, T>::convert(src.tiles[i][j].data[0].x));
            atomic<U>::addg(&dst_ptr[(row+0)*row_stride + (col+8)], base_types::convertor<U, T>::convert(src.tiles[i][j].data[1].x));
        }
        #pragma unroll
        for(int j = 0; j < src.width; j++) {
            int col = j*src.tile_size_col + (laneid / 4);
            atomic<U>::addg(&dst_ptr[(row+1)*row_stride + (col+0)], base_types::convertor<U, T>::convert(src.tiles[i][j].data[0].y));
            atomic<U>::addg(&dst_ptr[(row+1)*row_stride + (col+8)], base_types::convertor<U, T>::convert(src.tiles[i][j].data[1].y));
        }
        #pragma unroll
        for(int j = 0; j < src.width; j++) {
            int col = j*src.tile_size_col + (laneid / 4);
            atomic<U>::addg(&dst_ptr[(row+8)*row_stride + (col+0)], base_types::convertor<U, T>::convert(src.tiles[i][j].data[2].x));
            atomic<U>::addg(&dst_ptr[(row+8)*row_stride + (col+8)], base_types::convertor<U, T>::convert(src.tiles[i][j].data[3].x));
        }
        #pragma unroll
        for(int j = 0; j < src.width; j++) {
            int col = j*src.tile_size_col + (laneid / 4);
            atomic<U>::addg(&dst_ptr[(row+9)*row_stride + (col+0)], base_types::convertor<U, T>::convert(src.tiles[i][j].data[2].y));
            atomic<U>::addg(&dst_ptr[(row+9)*row_stride + (col+8)], base_types::convertor<U, T>::convert(src.tiles[i][j].data[3].y));
        }
    }
}
